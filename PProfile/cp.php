...
$connect= mysqli_connect('localhost','stone','stone','details'); // Establishing Connection with Server
 // Selecting Database from Server
	...
	if($Password && $repassword && $cur) 
	{
		...
		{
			$sql = mysqli_query($connect,"SELECT * FROM `details`.`plogin` WHERE `Username`='$Username'");
			if(mysqli_num_rows($sql) == 1)
			{
				...
				if($cur == $dbpassword)
				{
					if($query = mysqli_query($connect,"UPDATE `details`.`plogin` SET `Password` = '$Password' WHERE `plogin`.`Username` = '$Username'"))
					{
